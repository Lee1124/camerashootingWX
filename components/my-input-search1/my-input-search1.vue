...
<template>
	<view class="myInputSearch">
		<input type="text" v-model="inputSearch1Text" placeholder-style='color:#BBB' :placeholder="myInputSearch1Obj.myIputPlaceholder"
		...
		<view class="list-show-box" :class="{'list-show-box-top':myInputSearch1Obj.site=='top','list-show-box-bottom':myInputSearch1Obj.site=='bottom'}">
			<view class="list-show commonScrollStyle" v-show="myInputSearch1Obj.isShowListBox">
				...
				<view class="title title0" v-if="myInputSearch1Obj.isShowCreated">没有合适的{{myInputSearch1Obj.createName}}？ <text
	...
	export default {
		...
		methods: {
			...
			inputFocus() {
				...
				if (this.inputSearch1Text == '') {
					this.$emit('inputSearch1Focus', {
						val: this.inputSearch1Text,
					});
				}
				if (this.inputSearch1Text != this.choiceObj.name && this.choiceObj.name != '') {

...
async function uploadTasks(param) {
	...
	let cos = new COS({
		getAuthorization: function Authorization(params, callback) {
			ajax.axios({
				url: '/xlapi/CameraManage/CameraUploadImgManage/CameraUploadImgManage/GetSignature',
				data: {
					strModel: 'POST',
					pathname: '/', //只能传 /
					qheaderlist: '',
		...
		param.files.forEach((item, i) => {
			...
			cos.postObject({
				...
				if(data.statusCode == 200 || data.statusCode == 204 ){
					data.headers = {}
					data.headers.Location = 'http://xlfile-1256392453.cos.ap-chengdu.myqcloud.com'+param.key + `${ param.guid? '/'+sum:''}` + deml
				}

				param.callback && param.callback.call(that, a, data)
			})
...
function deteleImg(param) {
	...
	cos.deleteObject({
		Bucket: config.Bucket, // 替换为你的Bucket名称
		Region: config.Region, // 设置COS所在的区域，对应关系: 华南->cn-south, 华东->cn-east, 华北->cn-north
